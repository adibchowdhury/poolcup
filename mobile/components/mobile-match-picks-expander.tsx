...
type MobileMatchPicksExpanderProps = {
  poolId: string
  matchId: string
        ...
        <div className="flex min-w-0 flex-col items-end gap-0.5">
          ...
          {showAdvancePick ? (
            effectivePick != null && advanceName ? (
              <span className="inline-flex max-w-[8.5rem] items-center gap-1 text-[10px] text-muted-foreground">
                <TeamFlagImage
...
export function MobileMatchPicksExpander({
  poolId,
  matchId,
  scoringStyle,
  kickoffAt,
  ...
  const hasKickedOff = mounted && new Date(kickoffAt).getTime() <= nowMs
