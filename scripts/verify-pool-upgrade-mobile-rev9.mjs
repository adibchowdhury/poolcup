...
const probe = await page.evaluate(() => {
  ...
  const featureP = features?.querySelector('p')
  ...
  return {
    ...
    featureFontSize: featureP ? cs(featureP)?.fontSize : null,
    featureColor: featureP ? cs(featureP)?.color : null,
    featureIconSize: featureIcon
      ? Math.round(featureIcon.getBoundingClientRect().width)
      : null,
    cardPadding: card ? cs(card)?.paddingTop : null,
    order: {
    ...
    reassuranceAboveCta:
      reassurance &&
      cta &&
      reassurance.getBoundingClientRect().top <
        cta.getBoundingClientRect().top,
  }
})

const shot = resolve(outDir, 'pool-upgrade-mobile-rev9-390.png')
...
writeFileSync(
  resolve(outDir, 'pool-upgrade-mobile-rev9-report.json'),
