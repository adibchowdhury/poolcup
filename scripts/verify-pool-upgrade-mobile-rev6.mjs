...
const probe = await page.evaluate(() => {
  ...
  const close = Array.from(sheet?.querySelectorAll('button') ?? []).find((b) =>
  ...
  const benefits = Array.from(sheet?.querySelectorAll('p') ?? []).find((p) =>
    p.textContent?.includes('Keep forever'),
  )
  const featureTitle = features?.querySelector('p')
  const cs = (el) => (el ? getComputedStyle(el) : null)
  const closeCs = cs(close)
  const cardCs = cs(card)
  const ctaCs = cs(cta)
  return {
    closeW: close ? Math.round(close.getBoundingClientRect().width) : null,
    closeH: close ? Math.round(close.getBoundingClientRect().height) : null,
    closeBg: closeCs?.backgroundColor,
    closeIconSize: closeIcon
      ? Math.round(closeIcon.getBoundingClientRect().width)
      : null,
    featuresDisplay: features ? cs(features)?.display : null,
    featuresWidth: features
      ? Math.round(features.getBoundingClientRect().width)
      : null,
    featureColor: featureTitle ? cs(featureTitle)?.color : null,
    trustColor: trust ? cs(trust)?.color : null,
    reassuranceColor: reassurance ? cs(reassurance)?.color : null,
    benefitsColor: benefits ? cs(benefits)?.color : null,
    cardBorder: `${cardCs?.borderTopWidth} ${cardCs?.borderTopColor}`,
    cardOutline: `${cardCs?.outlineWidth} ${cardCs?.outlineColor} ${cardCs?.outlineStyle}`,
    ctaRadius: ctaCs?.borderRadius,
  }
})

const shot = resolve(outDir, 'pool-upgrade-mobile-rev6-390.png')
...
writeFileSync(
  resolve(outDir, 'pool-upgrade-mobile-rev6-report.json'),
