...
/** Admin-only: CSV export of pool leaderboard via export_pool_leaderboard. */
export async function GET(_request: Request, context: Ctx) {
  ...
  const { rows, error } = await fetchLeaderboardExportRows(
    admin,
    user.id,
    poolId,
  )
  if (error) {
    console.error('export_pool_leaderboard failed:', error)
  ...
  const filename = `${sanitizeFilenamePart(meta.name)}-leaderboard-${date}.csv`
  const csv = buildLeaderboardCsv(meta, rows)
