...
export async function listSyncableApiAmericanFootballEvents(
  ...
  return withTransientDbRetry(
    'listSyncableApiAmericanFootballEvents',
    async () => {
      let query = supabase
...
export async function syncAmericanFootballLiveScores(
  ...
  const windowProbe = await withTransientDbRetry(
    'syncAmericanFootballLiveScores:windowProbe',
    async () => {
      const { data, error } = await supabase
        .from('matches')
        .select('id')
      ...
      if (error) {
        throw new Error(
          `Failed to check american-football live window: ${error.message}`,
        )
      }
      return data
    },
  ...
  const liveRows = await withTransientDbRetry(
    'syncAmericanFootballLiveScores:liveMatches',
    async () => {
      const { data, error } = await supabase
        .from('matches')
        .select(
      ...
      if (error) {
        throw new Error(
          `Failed to load american-football live matches: ${error.message}`,
        )
      }
      return data
    },
