...
/** Neutral round for baseball (no tournament rounds). */
export const BASEBALL_MATCH_ROUND = 'regular'

const UPSERT_BATCH_SIZE = 40
const LOOKUP_BATCH_SIZE = 100
/** MLB regular season ~2.4k games; soft cap with headroom. */
const MAX_GAMES_PER_EVENT = 3000
const SCORE_BATCH_SIZE = 25

export type BaseballSportingEvent = {
  id: string
  name: string
  slug: string
  status: string
...
export type BaseballSyncResult = {
  eventId: string
  eventName: string
  leagueId: number
  season: number
...
export type SyncBaseballSummary = {
...
function toUtcIso(dateStr: string): string {
  ...
  if (!Number.isFinite(ms)) {
    throw new Error(`Invalid baseball game date: ${dateStr}`)
  }
...
function buildMatchRow(
  ...
  if (!isValidApiBaseballGameId(fixtureId)) return null

  const dateRaw = game.date?.trim()
  if (!dateRaw) return null
  const kickoffAt = toUtcIso(dateRaw)

  const statusShort = mapBaseballStatusToMatchStatus(game.status?.short)
  const runs = parseBaseballRuns(game)
  const isFinal =
    statusShort != null &&
    isBaseballFinalStatus(statusShort) &&
    runs != null
  ...
  return {
    ...
    row: {
      ...
      round: BASEBALL_MATCH_ROUND,
      group_name: null,
      status_short: statusShort,
      elapsed_minute: null,
      result_team1: runs?.resultTeam1 ?? null,
      result_team2: runs?.resultTeam2 ?? null,
      is_final: isFinal,
      advancing_team: null,
      match_number: null,
      team1_flag: null,
...
/**
 * Score newly finished baseball games via existing calculate_match_points RPC
...
export async function listSyncableApiBaseballEvents(
  supabase: SupabaseClient,
  options?: { eventId?: string | null },
): Promise<BaseballSportingEvent[]> {
  return withTransientDbRetry('listSyncableApiBaseballEvents', async () => {
    let query = supabase
      .from('sporting_events')
      .select(
        'id, name, slug, status, provider, provider_league_id, provider_season, event_type',
      )
      .eq('provider', API_BASEBALL_PROVIDER)
    ...
    if (error) {
      throw new Error(
        `Failed to load baseball sporting_events: ${error.message}`,
      )
    }
    return (data ?? []) as BaseballSportingEvent[]
  })
}

async function syncOneBaseballEvent(
  supabase: SupabaseClient,
  apiKey: string,
  event: BaseballSportingEvent,
): Promise<BaseballSyncResult> {
  const leagueId = Number(event.provider_league_id)
  const season = Number(event.provider_season)
  const base: BaseballSyncResult = {
    eventId: event.id,
  ...
  return withSyncJob(
    ...
    async () => {
      ...
      if (base.status === 'error') {
        throw new Error(
          base.errors[0] ?? `Baseball sync failed for ${event.name}`,
        )
      }
...
/**
 * Ingest MLB (api-baseball) games into matches for syncable sporting_events.
 */
export async function syncBaseballFromApi(
  supabase: SupabaseClient,
  apiKey: string,
  options?: { eventId?: string | null },
): Promise<SyncBaseballSummary> {
  const events = await listSyncableApiBaseballEvents(supabase, options)
  const results: BaseballSyncResult[] = []

  for (const event of events) {
    const result = await syncOneBaseballEvent(supabase, apiKey, event)
    results.push(result)
  }
...
/** Cheap live poll window — baseball games can run long (extras). */
const LIVE_PRE_KICKOFF_MINUTES = 5
const LIVE_MATCH_MAX_AGE_MINUTES = 480

export type SyncBaseballLiveSummary = {
...
export async function syncBaseballLiveScores(
  supabase: SupabaseClient,
  apiKey: string,
): Promise<SyncBaseballLiveSummary> {
  const empty = (
    skipped?: string,
  ): SyncBaseballLiveSummary => ({
    matchesChecked: 0,
    matchesUpdated: 0,
    matchesSkipped: 0,
    pointsScored: 0,
  ...
  const events = await listSyncableApiBaseballEvents(supabase)
  if (events.length === 0) {
    return empty('no_baseball_events')
  }
  ...
  // Cheap existence check before loading full rows / calling the API.
  const windowProbe = await withTransientDbRetry(
    'syncBaseballLiveScores:windowProbe',
    async () => {
      const { data, error } = await supabase
        .from('matches')
        .select('id')
      ...
      if (error) {
        throw new Error(
          `Failed to check baseball live window: ${error.message}`,
        )
      }
      return data
    },
  ...
  const liveRows = await withTransientDbRetry(
    'syncBaseballLiveScores:liveMatches',
    async () => {
      const { data, error } = await supabase
        .from('matches')
        .select(
      ...
      if (error) {
        throw new Error(
          `Failed to load baseball live matches: ${error.message}`,
        )
      }
      return data
    },
  ...
  const candidates = ((liveRows ?? []) as BaseballLiveMatchRow[]).filter(
    (match) => isValidApiBaseballGameId(match.fixture_id),
  )
  ...
  for (const match of candidates) {
    ...
    const runs = parseBaseballRuns(game)
    const apiLive = isBaseballLiveStatus(statusShort)
    let apiFinal = isBaseballFinalStatus(statusShort) && runs != null
    ...
    if (apiFinal && !canFinalizeMatchByKickoff(match.kickoff_at, nowMs)) {
      ...
      // Still allow live score/status updates without finalizing.
      apiFinal = false
    }

    // Ignore unchanged NS shells with no score yet.
    if (
      !apiFinal &&
      !apiLive &&
      statusShort === 'NS' &&
    ...
    const scoreChanged =
      runs != null &&
      (match.result_team1 !== runs.resultTeam1 ||
        match.result_team2 !== runs.resultTeam2)
    ...
    if (runs != null) {
      updatePayload.result_team1 = runs.resultTeam1
      updatePayload.result_team2 = runs.resultTeam2
    }
    ...
    if (
      ...
      await tryEmitUsSportScoreChange(
        ...
        { t1: runs.resultTeam1, t2: runs.resultTeam2 },
        statusShort,
      )
    }
