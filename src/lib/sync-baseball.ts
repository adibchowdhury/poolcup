...
export async function listSyncableApiBaseballEvents(
  ...
  if (error) {
    throw new Error(`Failed to load baseball sporting_events: ${error.message}`)
  }
...
export async function syncBaseballLiveScores(
  ...
  if (probeError) {
    throw new Error(`Failed to check baseball live window: ${probeError.message}`)
  }
  ...
  if (loadError) {
    throw new Error(`Failed to load baseball live matches: ${loadError.message}`)
  }
