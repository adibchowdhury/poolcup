...
CREATE OR REPLACE FUNCTION public.get_squad_leaderboard()
 RETURNS TABLE(rank integer, pool_id uuid, name text, avatar text, scoring_style text, member_count integer, total_points integer, avg_points numeric, is_mine boolean)
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
 SET search_path TO 'public'
